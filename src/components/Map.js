...
mapboxgl.accessToken = 'pk.eyJ1Ijoic2Fmd2FuZWZkZCIsImEiOiJjbTFjMjRscWMxd2hlMmpxeDJvYW4zNDhxIn0.baGxA7YI7TI1TfBSBf2hAg';
...
const Map = () => {
  ...
  const markers = [
    ...
    { name: 'Congo (République du Congo)', lng: 15.8277, lat: -0.2280 },
    { name: 'Congo (République Démocratique du Congo)', lng: 15.2663, lat: -4.4419 },
    { name: 'Côte d\'Ivoire', lng: -5.5471, lat: 7.5399 },
    { name: 'Djibouti', lng: 43.1500, lat: 11.5950 },
  ...
  useEffect(() => {
    if (map.current) return; // Si la carte est déjà initialisée, ne pas la recréer
    ...
    markers.forEach(marker => {
      new mapboxgl.Marker()
        ...
        .setPopup(new mapboxgl.Popup({ offset: 25 }) // Ajouter un popup pour chaque marqueur
          .setText(marker.name)) // Afficher le nom du pays ou de l'île dans le popup
